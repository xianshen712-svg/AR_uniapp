...
export default function (THREE) {
    ...
    GLTFLoader.prototype = {
        ...
        load: function (url, onLoad, onProgress, onError) {
            ...
            var loader = new FileLoader(that.manager);
            loader.setPath(this.path);
            loader.setResponseType('arraybuffer');
            if (that.crossOrigin === 'use-credentials') {
    ...
    GLTFParser.prototype.loadAccessor = function (accessorIndex) {
        ...
        return Promise.all(pendingBufferViews).then(function (bufferViews) {
            ...
            var array;
            var bufferAttribute; // The buffer is not interleaved if the stride is the item size in bytes.
            if (byteStride && byteStride !== itemBytes) {
                // Each "slice" of the buffer, as defined by 'count' elements of 'byteStride' bytes, gets its own InterleavedBuffer
                // This makes sure that IBA.count reflects accessor.count properly
                var ibSlice = Math.floor(byteOffset / byteStride);
                var ibCacheKey = 'InterleavedBuffer:' + accessorDef.bufferView + ':' + accessorDef.componentType + ':' + ibSlice + ':' + accessorDef.count;
                var ib = that.cache.get(ibCacheKey);
                if (!ib) {
                    array = new TypedArray(bufferView, ibSlice * byteStride, (accessorDef.count * byteStride) / elementBytes);

                    // Integer parameters to IB/IBA are in array elements, not bytes.
                    ib = new InterleavedBuffer(array, byteStride / elementBytes);
                    that.cache.add(ibCacheKey, ib);
                }
    ...
    GLTFParser.prototype.loadTexture = function (textureIndex) {
        ...
        if (source.bufferView !== undefined) {
            ...
            sourceURI = that.getDependency('bufferView', source.bufferView).then(function (bufferView) {
        ...
        return Promise.resolve(sourceURI)
            .then(function (sourceURI) {
                ...
                if (!loader) {
                    loader = textureExtensions[EXTENSIONS.MSFT_TEXTURE_DDS] ? that.extensions[EXTENSIONS.MSFT_TEXTURE_DDS].ddsLoader : textureLoader;
                }
                return new Promise(function (resolve, reject) {
    ...
    GLTFParser.prototype.assignTexture = function (materialParams, mapName, mapDef) {
        ...
        return this.getDependency('texture', mapDef.index).then(function (texture) {
            ...
            if (that.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM]) {
                var transform = mapDef.extensions !== undefined ? mapDef.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM] : undefined;
                if (transform) {
                    texture = that.extensions[EXTENSIONS.KHR_TEXTURE_TRANSFORM].extendTexture(texture, transform);
                }
            }
    ...
    GLTFParser.prototype.loadMaterial = function (materialIndex) {
        ...
        if (materialExtensions[EXTENSIONS.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS]) {
            ...
            pending.push(sgExtension.extendParams(materialParams, materialDef, that));
        } else if (materialExtensions[EXTENSIONS.KHR_MATERIALS_UNLIT]) {
            var kmuExtension = extensions[EXTENSIONS.KHR_MATERIALS_UNLIT];
            materialType = kmuExtension.getMaterialType();
            pending.push(kmuExtension.extendParams(materialParams, materialDef, that));
        } else {
            // Specification:
            ...
            if (metallicRoughness.baseColorTexture !== undefined) {
                pending.push(that.assignTexture(materialParams, 'map', metallicRoughness.baseColorTexture));
            }
            ...
            if (metallicRoughness.metallicRoughnessTexture !== undefined) {
                pending.push(that.assignTexture(materialParams, 'metalnessMap', metallicRoughness.metallicRoughnessTexture));
                pending.push(that.assignTexture(materialParams, 'roughnessMap', metallicRoughness.metallicRoughnessTexture));
            }
        }
        ...
        if (materialDef.normalTexture !== undefined && materialType !== MeshBasicMaterial) {
            pending.push(that.assignTexture(materialParams, 'normalMap', materialDef.normalTexture));
        ...
        if (materialDef.occlusionTexture !== undefined && materialType !== MeshBasicMaterial) {
            pending.push(that.assignTexture(materialParams, 'aoMap', materialDef.occlusionTexture));
        ...
        if (materialDef.emissiveTexture !== undefined && materialType !== MeshBasicMaterial) {
            pending.push(that.assignTexture(materialParams, 'emissiveMap', materialDef.emissiveTexture));
        }
    ...
    GLTFParser.prototype.loadGeometries = function (primitives) {
        ...
        function createDracoPrimitive(primitive) {
            return extensions[EXTENSIONS.KHR_DRACO_MESH_COMPRESSION].decodePrimitive(primitive, that).then(function (geometry) {
                return addPrimitiveAttributes(geometry, primitive, that);
            });
        }
        var pending = [];
        for (var i = 0, il = primitives.length; i < il; i++) {
            ...
            if (cached) {
                ...
                if (primitive.extensions && primitive.extensions[EXTENSIONS.KHR_DRACO_MESH_COMPRESSION]) {
                    ...
                    geometryPromise = addPrimitiveAttributes(new BufferGeometry(), primitive, that);
                }
    ...
    GLTFParser.prototype.loadMesh = function (meshIndex) {
        ...
        return Promise.all(pending).then(function (originalMaterials) {
            return that.loadGeometries(primitives).then(function (geometries) {
                var meshes = [];
    ...
    GLTFParser.prototype.loadCamera = function (cameraIndex) {
        ...
        if (cameraDef.type === 'perspective') {
            camera = new PerspectiveCamera(_Math.radToDeg(params.yfov), params.aspectRatio || 1, params.znear || 1, params.zfar || 2000000);
            ...
            for (var i = 0, il = nodes.length; i < il; i++) {
                ...
                if (outputAccessor.normalized) {
                    ...
                    if (outputArray.constructor === Int8Array) {
                        ...
                        scale = 0.00003051850947599719;
                    } else if (outputArray.constructor === Uint16Array) {
                        scale = 0.000015259021896696422;
                    } else {
    ...
    GLTFParser.prototype.loadNode = function (nodeIndex) {
        ...
        return (function () {
            ...
            if (nodeDef.mesh !== undefined) {
                pending.push(
                    that.getDependency('mesh', nodeDef.mesh).then(function (mesh) {
                        var node;
            ...
            if (nodeDef.camera !== undefined) {
                pending.push(that.getDependency('camera', nodeDef.camera));
            }
            if (nodeDef.extensions && nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL] && nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].light !== undefined) {
                pending.push(that.getDependency('light', nodeDef.extensions[EXTENSIONS.KHR_LIGHTS_PUNCTUAL].light));
            }
            return Promise.all(pending);
    ...
    GLTFParser.prototype.loadScene = (function () {
        ...
        return function loadScene(sceneIndex) {
            ...
            for (var i = 0, il = nodeIds.length; i < il; i++) {
                pending.push(buildNodeHierachy(nodeIds[i], scene, json, that));
            }
            return Promise.all(pending).then(function () {
                return scene;
            });
